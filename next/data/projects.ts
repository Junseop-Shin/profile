    ...
    sections: [
      {
        heading: "주요 작업",
        items: [
          "멈춰 있던 아키텍처 논의를 정리해 전체 설계와 플랜 수립 — 공통 구조 + 분리 빌드·배포 + 기본앱 제공 체계",
          "설계를 파트로 쪼개 팀에 배분하고, 남은 프론트엔드와 이벤트 파이프라인을 직접 담당",
      ...
      },
      {
        heading: "비즈니스 분리 구조",
        items: [
          "새 비즈니스가 들어오면 기본 포맷을 복사해 분리 생성하는 방식",
          "DB — 비즈니스별 스키마 단위로 분리",
          "API 서버 — 비즈니스별 컨테이너로 분리 생성",
          "프론트 — 웹/모바일만 분리하고 앱 간에는 코드를 공유",
          "분리 빌드·배포를 기본으로 하되, 통으로 빌드·배포하는 길도 열어둠",
        ],
      },
      {
        heading: "프론트엔드 프레임워크",
        items: [
          "편집 모드 — 화면을 직접 구성하는 편집/뷰 모드 전환 및 편집 상태 관리",
          "UI 배치 — 위젯·레이아웃 배치 구조와 배치 정보 저장",
          "locale — 다국어 리소스 구조 및 언어 전환 체계",
          "상태 관리 — 공통/비즈니스 경계에 맞춘 전역 상태 설계",
          "API 레이어 — 공통 클라이언트, 인증·에러 처리, 서버 상태 캐싱",
          "디자인 시스템 및 디자인 문서 적용",
        ],
      },
      {
        heading: "이벤트 파이프라인 (Go)",
        items: [
          "수신 어댑터 → 판단 코어 → 실행기로 이어지는 3단 구조",
          "이벤트 유실 방지를 위한 멱등 처리 + DLQ",
          "정기보고 수집 경로와 Event Hubs 네임스페이스를 공유하되 별도 토픽으로 분리",
          "Go 선택 — 가벼운 동시성으로 GIL 우회(멀티프로세스 + VMSS)가 불필요해지고, 정적 컴파일 단일 바이너리로 컨테이너 이미지가 수백 MB → 수십 MB",
          "처리량이 모자라면 쿠버네티스 인스턴스 증설로 대응",
        ],
      },
    ],
  },
