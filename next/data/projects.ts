...
export const projects: Project[] = [
  ...
  },
  {
    ...
    summary:
      "여러 서비스가 한 서버에 얽혀 있던 기존 IoT 플랫폼을, 비즈니스별로 분리해 얹는 구조로 다시 세우는 차세대 버전. 한 달째 멈춰 있던 아키텍처 논의를 정리해 전체 설계와 플랜을 수립하고, 설계를 파트로 쪼개 팀에 배분한 뒤 프론트엔드와 이벤트 파이프라인을 직접 맡았다. 이후 빌드·배포 파이프라인과 모니터링도 맡아 네 축을 담당하고 있다",
    diagram: {
      ...
      caption:
        "비즈니스 식별자는 business-api-N · biz_N으로 익명 표기했습니다. 전체 아키텍처 정리는 본인이 맡았고, 직접 설계·구현한 파트는 프론트엔드 · 이벤트 파이프라인 · 빌드/배포 파이프라인 · 모니터링입니다.",
    },
    metrics: [
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "모니터링을 세워 서비스 헬스체크·로그·리소스 상태값을 ClickHouse 한 곳에 모으고 관리자 화면까지 붙였다. 개발 환경 배포·연동까지 마쳤다",
        ],
      },
      {
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "관리자 앱에 모니터링 메뉴와 상태·로그·리소스 세 화면을 붙여, 따로 만든 모니터링이 실제로 보이는 자리를 만들었다",
  ...
  },
  {
    ...
    subtitle: "열려 있던 SSH 포트를 닫는 것에서 시작한 CI/CD",
    period: "2026.07 ~",
    stack: [
      "GitHub Actions",
      "Azure Entra ID",
    ...
    thumbnail: "/platform-v2-thumb.png",
    summary:
      "서비스를 어떻게 올릴지가 정해지지 않은 채로 붙을 서비스만 늘어나던 상황에서 맡게 된 일. 담당이 따로 있었지만 진척이 없어 직접 잡았다. 배포하려고 열어둔 SSH 포트로 공격을 받은 적이 있어서, 그 포트를 닫고도 배포가 되게 만드는 것이 출발점이었다",
    metrics: [
      { label: "배포용 SSH 포트", value: "닫음" },
      { label: "상시 점유 러너", value: "없음" },
      { label: "이 방식으로 올라가는 것", value: "7종" },
      { label: "기빌드 커밋 재배포", value: "수 초" },
    ],
    sections: [
      {
        heading: "접속 경로를 세 번 바꿨다",
        items: [
          "처음에는 배포하려고 VM의 SSH 포트를 열어두고 있었고, 그 포트로 공격이 들어온 적이 있다. 닫는 것 자체는 바로 할 수 있지만 닫으면 배포할 방법이 없어지는 게 문제였다",
          "두 번째로 self-hosted runner를 VM 안에 두고 러너가 안에서 배포하게 바꿨다. 밖에서 VM으로 들어오는 경로를 없애고, VM 안에서 밖으로 나가 받아오게 한 것이다",
          "이 방식은 오래 가지 않았다. self-hosted runner가 유료로 바뀌었고, 그걸 떠나서도 하루 몇 번의 배포를 위해 VM 자원을 상시 물고 있어야 했다. 그러면서 빌드는 느렸다",
          "세 번째로 Entra ID와 OIDC를 도입했다. CI가 그때그때 받은 단기 토큰으로 클라우드에 인증하고, VM에는 접속하는 대신 클라우드 API로 명령을 실행시킨다. 포트도 닫혀 있고, 상시로 도는 러너도 없고, 어딘가에 보관해야 할 키도 없다",
        ],
      },
      {
        heading: "흩어진 것을 한곳으로",
        items: [
          "워크플로우를 레포마다 복사해 두지 않고 별도 레포 하나에 모아 재사용하게 했다. 규칙을 바꿀 일이 생기면 그 레포만 고친다",
          "각 레포에 흩어져 있던 secret과 variable도 같이 모았다. 여기는 아직 절반이다. 지금은 CI 쪽에 값을 두고 있고 Key Vault로 옮길 생각이다",
          "Terraform이 들고 있는 상태 파일과 배포할 도커 이미지는 둘 다 Blob Storage에 둔다. 이미 쓰고 있는 스토리지라 컨테이너 레지스트리를 따로 두거나 상태 파일 둘 곳을 새로 찾을 필요가 없었다",
        ],
      },
      {
        heading: "무엇을 코드로 내리고 무엇을 통일할 것인가",
        items: [
          "인프라를 전부 Terraform으로 내리지는 않았다. 인스턴스 관리가 필요하거나 관리형 서비스인 경우에만 쓴다. VM 한 대 올리는 일까지 코드로 만들면 관리할 것만 늘어난다고 봤다",
          "실행 방식은 통일했다. pm2로 도는 것, jar로 도는 것, 도커로 도는 것이 섞여 있던 것을 전부 docker compose로 맞췄다",
          "서비스마다 올리는 법이 다르면 배포 스크립트도 서비스 수만큼 생긴다. 통일해두니 스크립트가 하나로 줄었고, 모니터링에서 로그를 모을 때도 경로가 서비스마다 갈리지 않았다",
          "빌드와 배포는 반대로 갈라 뒀다. 같은 커밋을 다시 실행하면 이미지가 이미 있으니 검사와 빌드를 건너뛰고 배포만 돈다. 되돌리는 일이 부담스럽지 않아진다",
        ],
      },
      {
        heading: "지금 올라간 것과 남은 것",
        items: [
          "프론트엔드, 공통 API 서버, 모니터링 게이트웨이와 에이전트, MQTT 브로커, MongoDB, API Management까지 이 방식으로 올라간다",
          "남은 것은 이벤트·텔레메트리 파이프라인과 Event Hubs, 그리고 나머지 API 서버들이다",
        ],
      },
    ],
  },
  {
    ...
    company: "주식회사 티앤엠테크",
    title: "v2 모니터링",
    subtitle: "ClickHouse 한 곳에 헬스체크·로그·리소스를 모으고 화면까지",
    period: "2026.09 ~",
    stack: [
      "ClickHouse",
      "OpenTelemetry",
    ...
    thumbnail: "/platform-v2-thumb.png",
    summary:
      "개발 환경에서 디스크가 찬 걸 두 번 놓치고, 로그를 보려면 VM에 들어가 파일을 찾아야 하던 상황에서 시작했다. 서비스가 살아 있는지, 로그에 무엇이 찍혔는지, 리소스가 얼마나 쓰이는지 셋을 ClickHouse 한 곳에 모으고, 조회 API와 관리자 화면 메뉴까지 붙여 닫았다",
    metrics: [
      { label: "수집 대상", value: "VM 3대 · 서비스 12종" },
      { label: "적재량", value: "압축 후 약 200MB/일" },
      { label: "보존", value: "로그 14일 · 트레이스 7일 · 지표 30일" },
      { label: "화면", value: "상태 · 로그 · 리소스" },
    ],
    sections: [
      {
        heading: "왜 ClickHouse 하나인가",
        items: [
          "모을 것을 셋으로 정했다. 서비스가 지금 살아 있는지, 로그에 무엇이 찍혔는지, VM과 클라우드 리소스가 얼마나 쓰이고 있는지다",
          "OpenTelemetry로 모으면 보통 지표는 Prometheus, 로그는 Loki, 트레이스는 또 다른 백엔드로 셋이 갈린다. ClickHouse는 그걸 한 서비스가 다 받는다. 운영할 것이 셋에서 하나로 줄고, 꺼낼 때도 같은 방식으로 꺼낸다",
          "로그를 담기에 맞는 저장소라는 점도 컸다. 컬럼 지향이라 반복이 많은 로그가 잘 압축되고, 조회할 때 필요한 컬럼만 읽어서 인덱스를 따로 두지 않아도 기간 검색이 버틴다. 압축 후 하루 200MB 남짓이라 보존 기간을 넉넉히 잡아도 부담이 없었다",
          "오픈소스라 라이선스 비용도 없다. 스키마는 직접 설계하지 않고 수집기가 만드는 기본값을 썼다. 검증된 스키마를 쓰는 대신 매핑을 직접 유지하는 부담을 지지 않기로 했다",
          "보존 기간은 데이터 종류마다 다르게 걸었다. 로그 14일, 트레이스 7일, 지표 30일이다. 트레이스는 이벤트 파이프라인 한정이라 짧게 잡아도 되고, 지표는 양이 가벼워서 길게 두고 추이를 본다",
        ],
      },
      {
        heading: "에이전트와 게이트웨이를 나눈 이유",
        items: [
          "VM마다 에이전트를 두되 그 VM 안에서만 보이는 것, 그러니까 파일 로그와 호스트 지표만 걷게 했다. 에이전트는 그걸 게이트웨이로 넘기기만 한다",
          "ClickHouse에 쓰는 지점은 게이트웨이 하나다. 자격증명이 한 곳에만 있고, 포트도 지정한 사설 IP에만 열어 뒀다. VM이 늘어도 쓰기 경로는 늘지 않는다",
          "헬스체크와 클라우드 리소스 지표는 게이트웨이가 직접 주기적으로 찔러서 가져온다. 각 서비스의 헬스 엔드포인트와 클라우드 모니터링 API를 폴링하는 방식이다",
          "로그 수집은 일부러 섞었다. 이벤트 파이프라인은 이미 계측이 있어 엔드포인트 주소만 바꿔 직접 보내고, 나머지는 파일을 따라 읽는다. 앱 코드를 고치지 않고 시작할 수 있는 쪽을 골랐다",
        ],
      },
      {
        heading: "API와 화면까지 닫았다",
        items: [
          "조회 API를 새 서비스로 만들지 않고 공통 API 서버의 모듈로 넣었다. 인증과 권한, 타입 생성이 이미 깔린 레일 위에 얹는 편이 규모에 맞았다",
          "관리자 앱에 모니터링 메뉴를 만들고 그 아래에 상태·로그·리소스 세 화면을 뒀다. 앞에서 정한 셋이 그대로 화면 셋이 된다",
          "화면끼리 이어지게 했다. 상태 카드를 누르면 그 서비스의 로그로, 로그 행의 추적 ID를 누르면 드로어에 구간 목록이 뜬다",
          "조회 SQL은 서비스 계층의 고정 문자열이고 필터는 파라미터로만 바인딩한다. 화면이 SQL을 만들지 않는다는 선을 그었다. 계정도 쓰기용과 읽기용을 나눠 조회 API가 쓰기 권한을 들지 않게 했다",
          "트레이스 전용 화면은 만들지 않았다. 추적이 이벤트 파이프라인 한정이라 전용 화면은 과하다고 봤다",
          "배포도 직접 만든 배포 파이프라인으로 옮겼다. 처음엔 수동으로 파일을 복사해 올렸는데, 자산 위치와 컨테이너 이름을 플랫폼 규칙에 맞추면서 모니터링도 다른 서비스와 같은 방식으로 배포된다",
        ],
      },
      {
        heading: "모니터링이 죽었을 때가 제일 중요하다",
        items: [
          "ClickHouse 접속이 실패해도 조회 API가 에러를 내지 않는다. 상태는 전 항목을 알 수 없음으로 두고, 나머지는 빈 결과에 지금 정상이 아니라는 표시를 붙여 돌려준다. 모니터링 자체의 장애가 화면에 보이지 않으면 전부 정상으로 읽히기 때문이다",
          "같은 이유로 마지막 값이 몇 분 이상 안 들어오면 정상이 아니라 알 수 없음으로 떨어뜨린다. 데이터가 안 오는 것과 상태가 좋은 것은 다르다",
      ...
      },
      {
        ...
        items: [
          ...
          "관리형 리소스는 APIM만 로그를 가져오고 나머지는 지표만 본다. 양 끝이 우리 코드인 구간은 간접적으로 보는 것으로 충분하다고 판단했고, 필요해지면 그때 늘리기로 했다",
        ],
      },
    ],
  },
