    ...
    sections: [
      {
        ...
        items: [
          ...
          "두 번째로 self-hosted runner를 VM 안에 두고 러너가 안에서 배포하게 바꿨다. 밖에서 들어오는 문을 닫고 안에서 당겨오는 방향으로 뒤집은 것이다",
      ...
      },
      {
        ...
        items: [
          "인프라를 전부 Terraform으로 내리지는 않았다. 같은 것을 여러 개 만들어야 하거나 관리형 서비스인 경우에만 쓴다. VM 한 대 올리는 일까지 코드로 만들면 관리할 것만 늘어난다고 봤다",
          "실행 방식은 통일했다. pm2로 도는 것, jar로 도는 것, 도커로 도는 것이 섞여 있던 것을 전부 docker compose로 맞췄다",
          "서비스마다 올리는 법이 다르면 배포 스크립트도 서비스 수만큼 생긴다. 통일해두니 스크립트가 하나로 줄었고, 모니터링에서 로그를 걷을 때도 같은 자리에서 걷게 됐다",
    ...
    sections: [
      {
        ...
        items: [
          ...
          "셋을 저장소 세 개에 나눠 담고 싶지 않았다. 로그도 지표도 추적도 같은 곳에 넣고 같은 방식으로 꺼낼 수 있다는 것이 ClickHouse를 고른 이유였다. 오픈소스라 라이선스 비용도 없다",
          "스키마는 직접 설계하지 않고 수집기가 만드는 기본값을 썼다. 검증된 스키마를 쓰는 대신 매핑을 직접 유지하는 부담을 지지 않기로 했다",
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          "ClickHouse를 별도 VM이 아니라 MongoDB가 도는 VM에 얹었다. 개발 단계라 비용을 0으로 두려는 선택이었고, 앱 VM이 죽어도 모니터링은 살아 있다는 이점이 같이 따라왔다",
          "대신 메모리 상한을 반드시 걸어야 했다. VM 전체가 4GB가 안 되는데 기존 프로세스와 공존하므로 서버 메모리와 캐시를 묶고 백그라운드 풀도 줄였다. 상한 없이 올리면 이웃을 굶긴다",
