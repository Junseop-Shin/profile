...
export const projects: Project[] = [
  ...
  },
  {
    ...
    summary:
      "여러 서비스가 한 서버에 얽혀 있던 기존 IoT 플랫폼을, 비즈니스별로 분리해 얹는 구조로 다시 세우는 차세대 버전. 한 달째 멈춰 있던 아키텍처 논의를 정리해 전체 설계와 플랜을 수립하고, 설계를 파트로 쪼개 팀에 배분한 뒤 프론트엔드와 이벤트 파이프라인을 직접 맡았다. 이후 빌드·배포 파이프라인과 관측 스택도 맡아 네 축을 담당하고 있다",
    diagram: {
      ...
      caption:
        "비즈니스 식별자는 business-api-N · biz_N으로 익명 표기했습니다. 전체 아키텍처 정리는 본인이 맡았고, 직접 설계·구현한 파트는 프론트엔드 · 이벤트 파이프라인 · 빌드/배포 파이프라인 · 관측 스택입니다.",
    },
    metrics: [
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          "레포마다 제각각이던 빌드·배포를 공용 파이프라인으로 모으고, 전 레포가 같은 모양을 갖도록 CI/CD 컨벤션을 문서로 고정했다",
          "관측 스택을 세워 서비스 건전성·로그·VM 자원을 관리자 화면에서 보게 했다. 개발 환경 배포·연동까지 마쳤다",
        ],
      },
      {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: "목업이 실 API로 바뀌기까지",
        items: [
          "앞 절의 방식대로 목업을 먼저 세워 두고, 백엔드 계약이 나오는 순서대로 갈아끼웠다. 공통 도메인과 감사 로그를 시작으로 시설관리·청소관리·근태관리 웹앱까지 실 API로 배선을 마쳤다. 감사 로그는 목록 조회만이 아니라 CSV 내보내기를 서버가 만들게 해 브라우저가 전량을 받아 말지 않게 했다",
          "레거시 5개 앱의 화면을 3개 비즈니스 · 5개 플러그인으로 이식했다. 이 단계에서 플러그인 구조가 실제로 견디는지가 드러났고, 앱마다 다른 것이 정말 데이터로 흡수되는지를 확인하는 시험대가 됐다",
          "플러그인 정의를 DB로 옮기는 작업을 세 단계에 걸쳐 끝냈다. 앱 목록이 코드 카탈로그에 매여 있던 것을 풀고, 페이지 바디를 실 API로 옮기고, 마지막에는 매니페스트 합성이 DB에 있는 다국어 사전·스타일·리전 구성까지 소비하게 했다. 신규 앱 온보딩이 매니페스트 작성과 DB 구성으로 좁혀진다는 설계 주장이 여기서 실제로 성립했다",
          "앱 전용 서브도메인으로 들어오면 그 앱으로 스코프가 잡히는 테넌트 호스트 라우팅을 붙였다. 같은 배포가 호스트에 따라 다른 앱으로 보인다",
          "API 매핑 키에 타입 바인딩을 걸고, 백엔드 응답 스키마와 어긋나는 지점을 갭 리포트로 뽑게 했다. 계약이 바뀌었는데 화면이 조용히 빈 값을 그리는 상황을 빌드 시점으로 끌어올린 것이다",
          "관리자 앱에 모니터링 상태·로그·리소스 세 화면을 붙여, 따로 만든 관측 스택이 실제로 보이는 자리를 만들었다",
          "배포 방식을 pm2에서 컨테이너로 바꾸고 공용 빌드·배포 파이프라인에 편입했다. 이어서 서버 쪽 파일 로그를 도입해 프론트도 다른 서비스와 같은 방식으로 로그가 걷히게 했다",
        ],
      },
    ],
  },
  {
    ...
    sections: [
      ...
      {
        heading: "1차 구현을 끝낸 뒤에 다시 손댄 것",
        items: [
          "비즈니스마다 파이프라인 한 세트를 돌리는 모양으로 재구조화하면서 프로세스를 여섯에서 둘로 줄였다. 비즈니스가 늘 때 늘어나는 프로세스 수가 운영 부담의 실체였기 때문에, 여기를 줄이는 것이 곧 비즈니스 추가 비용을 줄이는 일이었다",
          "게이트웨이와 협의한 프로토콜을 끝까지 구현했다. 먼저 구현과 명세의 차이를 갭 카탈로그로 전부 적어 놓고, 프로토콜을 바꾸지 않고 메울 수 있는 것부터 닫은 뒤 남은 것을 처리하는 순서로 갔다. 무엇이 아직 안 되는지가 목록으로 보이는 상태를 먼저 만든 것이 실제로는 제일 컸다",
          "거절이 기록에 남지 않는 자리가 있었다. 요청과 응답을 감사 기록으로 남겨, 게이트웨이가 명령을 거부한 것과 명령이 아예 도달하지 않은 것을 구분할 수 있게 했다",
          "이름을 한 번 정리했다. 코드의 Processor를 EventHandler로 바꾸고, DB 표 이름도 판정과 실행 축에 주체·규칙·기록 축을 교차시킨 격자로 통일했다. 판정 상태가 알람·게이트웨이 접속·청구 확정별로 흩어져 있던 것도 한 표로 모았다. 새로 들어온 사람이 표 이름만 보고 그것이 무엇인지 알 수 있게 하려는 것이었다",
          "SQL에 스키마 이름을 박아 두면 안 된다는 것을 배포 사고로 배웠다. 비즈니스별로 스키마가 갈리는 구조인데 쿼리가 스키마를 고정하고 있어서, 한 비즈니스의 배포가 다른 비즈니스의 표를 보고 있었다",
        ],
      },
    ],
  },
  {
    slug: "platform-v2-deploy",
    type: "work",
    company: "주식회사 티앤엠테크",
    title: "v2 빌드·배포 파이프라인",
    subtitle: "레지스트리도 SSH 키도 없이 굴리는 CI/CD",
    period: "2026.07 ~",
    stack: [
      "GitHub Actions",
      "Docker",
    ...
    thumbnail: "/platform-v2-thumb.png",
    summary:
      "서비스 레포마다 제각각이던 빌드·배포를 공용 레포 하나로 모은 파이프라인. 컨테이너 레지스트리를 따로 두지 않고 Azure Blob을 이미지 저장소로 쓰고, VM 접속은 SSH 키 없이 Azure OIDC와 run-command로 한다. 처음엔 이벤트 파이프라인 안에 있던 배포 스크립트였는데 붙일 레포가 늘면서 재사용 워크플로우로 떼어냈다",
    metrics: [
      { label: "올라간 서비스", value: "7종" },
      { label: "분리한 신원", value: "4개" },
      { label: "클라이언트 암호", value: "0개" },
      { label: "이미 빌드된 커밋 재배포", value: "수 초" },
    ],
    sections: [
      {
        heading: "레지스트리를 두지 않았다",
        items: [
          "이미지는 빌드해서 tar로 말아 Blob의 docker-images/서비스명/커밋SHA 경로에 올리고, VM이 자기 관리 ID로 내려받아 docker load 한다. 개발 단계에서 레지스트리 하나를 더 운영하고 과금하는 것보다 이미 있는 스토리지 계정을 쓰는 쪽이 쌌다",
          "빌드와 배포를 갈라 뒀다. 같은 커밋을 다시 실행하면 이미지가 이미 있으므로 검사와 빌드를 통째로 건너뛰고 배포만 돈다. 롤백이 사실상 재배포 한 번이 된다",
          "다만 이 건너뛰기가 성립하는 범위는 정확히 적어 뒀다. 키가 서비스명과 커밋 SHA뿐이라 base 이미지 태그가 움직이면 같은 SHA라도 다른 바이너리가 나올 수 있다. 재현성이 중요해지면 base를 digest로 고정해야 한다",
          "저장소가 불변성을 강제하지도 않는다. 업로드 직전에 존재 확인을 한 번 더 하지만 그건 호출 규율이지 WORM이 아니고, 쓰기 권한을 가진 신원이면 교체가 가능하다. 90일 수명주기가 지나면 운영 중인 이미지의 원본 tar도 사라져서, 그 뒤의 재배포는 그때 그 바이너리가 아니다",
        ],
      },
      {
        heading: "비밀번호를 한 개도 만들지 않았다",
        items: [
          "빌드·배포·인프라·VM 네 신원을 나누고 전부 OIDC나 관리 ID로 했다. 빌드 신원으로는 VM에 명령을 못 보내고, 배포 신원으로는 이미지를 못 만들며, 인프라 신원은 VM도 이미지도 못 건드린다",
          "권한 범위를 스토리지 계정이 아니라 컨테이너 단위로, 구독이나 리소스 그룹이 아니라 VM 한 대 단위로 줬다. 범위를 넓게 주면 편하지만 뚫렸을 때 번지는 폭이 그만큼 넓어진다",
          "Terraform state는 이미지와 다른 컨테이너에 뒀다. 이미지 컨테이너는 VM 세 대가 읽기 권한을 갖고 있어서, 같은 곳에 두면 VM 하나가 뚫릴 때 state가 딸려 나간다",
          "배포 대상 VM 목록은 이 레포가 허용목록으로 갖는다. 부르는 쪽이 compose와 VM 조합을 자유롭게 고를 수 있으면 dispatch 토큰 하나로 임의의 컨테이너를 임의의 VM에 root로 올릴 수 있기 때문이다",
        ],
      },
      {
        heading: "단계를 늘리지 않는다",
        items: [
          "이미지 확인, 검사, 빌드·업로드, 배포 네 단계가 전부다. 같은 판단을 두 곳에서 하지 않기로 정하고, 재사용 워크플로우 안에 중복돼 있던 확인 때문에 로그인과 Blob 조회가 두 번씩 돌던 것을 걷어냈다",
          "빌드·배포가 PR에서 도는 검사 워크플로우를 그대로 호출한다. 따로 두면 테스트가 빨간 커밋의 이미지가 저장소에 올라가고, 그 뒤로는 이미 있는 이미지로 취급돼 조용히 배포된다",
          "레포와 서비스의 허용목록은 첫 단계에 뒀다. 빌드 단계에만 두면 이미지가 이미 있을 때 그 단계를 건너뛰므로 검증이 통째로 빠진다",
          "건너뛴 단계는 실패가 아니라는 점에 한 번 걸렸다. 앞 단계가 실패해 검사가 건너뛰어지면, 실패만 막는 조건으로는 검사 없이 배포까지 가는 경로가 열린다",
          "main push 트리거는 두지 않았다. 스쿼시 머지된 main은 방금 PR에서 검사한 것과 같은 트리라 중복이고, 머지가 곧 배포가 되면 배포 시점을 사람이 못 정한다",
        ],
      },
      {
        heading: "알고 감수한 것",
        items: [
          "포맷 검사는 바뀐 파일만 본다. 이미 쌓인 위반을 한 번에 재포맷하면 이력이 흐려지고 리뷰가 불가능해지기 때문인데, 대신 기준 브랜치와 같은 지점에서 돌리면 검사 대상이 0개가 된다. 배포 경로의 포맷 검사에는 기대지 않는 게 맞다",
          "배포 경로에서는 이미지 기동 확인을 끈다. 바로 뒤에 진짜 빌드가 돌아 중복이라서인데, 그 단계를 끄면 컨테이너를 띄워 보는 확인도 같이 빠진다. PR을 거치지 않은 커밋을 수동 배포하면 빌드는 되는데 뜨지는 않는 이미지가 VM까지 가서야 드러난다",
          "무중단 배포는 아직 없다. 포트만 blue/green으로 예약해 두고 운영 VM이 생길 때 붙이기로 미뤘다",
        ],
      },
    ],
  },
  {
    slug: "platform-v2-monitoring",
    type: "work",
    company: "주식회사 티앤엠테크",
    title: "v2 관측 스택",
    subtitle: "ClickHouse + OpenTelemetry 2계층 수집과 관리자 화면",
    period: "2026.09 ~",
    stack: [
      "ClickHouse",
      "OpenTelemetry",
    ...
    thumbnail: "/platform-v2-thumb.png",
    summary:
      "서비스가 살아 있는지, 로그에 무엇이 찍혔는지, VM 자원이 얼마나 남았는지를 관리자 화면에서 보게 만든 관측 스택. 수집은 OpenTelemetry Collector 2계층, 저장은 ClickHouse 단일 노드, 조회 API는 공통 API 서버의 모듈로, 화면은 관리자 앱 플러그인으로 붙였다. 개발 환경 배포와 연동까지 마쳤다",
    metrics: [
      { label: "수집 대상", value: "VM 3대 · 서비스 12종" },
      { label: "적재량", value: "압축 후 약 200MB/일" },
      { label: "보존", value: "로그 14일 · 트레이스 7일 · 지표 30일" },
      { label: "ClickHouse 메모리 상한", value: "1.5GB" },
    ],
    sections: [
      {
        heading: "에이전트와 게이트웨이를 나눈 이유",
        items: [
          "VM마다 에이전트를 두고, 그 VM 안에서만 보이는 것을 걷게 했다. 파일 로그와 호스트 지표가 그렇다. 에이전트는 그걸 게이트웨이로 넘기기만 한다",
          "ClickHouse에 쓰는 지점은 게이트웨이 하나다. 자격증명이 한 곳에만 있고, 8123 포트는 앱 VM 사설 IP만 열어 뒀다. 에이전트를 VM마다 늘려도 쓰기 경로는 늘지 않는다",
          "로그 수집은 일부러 섞었다. 이벤트 파이프라인은 이미 OpenTelemetry 계측이 있어 엔드포인트 주소만 바꿔 직접 보내고, 나머지는 파일을 따라 읽는다. 앱 코드를 고치지 않고 시작할 수 있는 쪽을 골랐다",
          "ClickHouse 스키마는 직접 설계하지 않고 exporter 기본값을 썼다. 검증된 스키마를 쓰는 대신 매핑을 직접 유지하는 부담을 지지 않기로 한 것이다",
        ],
      },
      {
        heading: "관측 스택이 죽었을 때가 제일 중요하다",
        items: [
          "ClickHouse 접속이 실패해도 조회 API가 5xx를 내지 않는다. 상태는 전 항목을 알 수 없음으로 두고, 나머지는 빈 결과에 degraded 헤더를 붙여 돌려준다. 모니터링 자체의 장애가 화면에 보이지 않으면 전부 정상으로 읽히기 때문이다",
          "상태 판정도 마지막 값이 3분 이상 없으면 정상이 아니라 알 수 없음으로 떨어뜨린다. 데이터가 안 오는 것과 상태가 좋은 것은 다르다",
          "ClickHouse 자신의 헬스도 수집 대상에 넣었다. 데이터가 없다는 것이 곧 죽었다는 뜻이 되는 컴포넌트라서다",
        ],
      },
      {
        heading: "남의 VM에 얹으면서 지킨 것",
        items: [
          "ClickHouse를 별도 VM이 아니라 MongoDB가 도는 VM에 얹었다. 개발 단계라 비용을 0으로 두려는 선택이었고, 앱 VM이 죽어도 관측이 살아 있다는 이점이 같이 따라왔다",
          "대신 메모리 상한을 반드시 걸어야 했다. VM 전체가 3.8GB인데 mongod와 공존하므로 서버 메모리 1.5GB, 마크 캐시 256MB로 묶고 백그라운드 풀도 줄였다. 상한 없이 올리면 ClickHouse가 이웃을 굶긴다",
          "계정을 INSERT용과 SELECT용으로 나눴다. 조회 API가 쓰기 권한을 들고 있을 이유가 없다",
          "조회 SQL은 서비스 계층의 고정 문자열이고 필터는 파라미터로만 바인딩한다. 화면이 SQL을 만들지 않는다는 선을 그었다",
        ],
      },
      {
        heading: "화면까지 닫았다",
        items: [
          "조회 API를 새 서비스로 만들지 않고 공통 API 서버의 모듈로 넣었다. 인증과 카탈로그, 타입 생성이 이미 깔린 레일 위에 얹는 편이 규모에 맞았다",
          "관리자 앱에 상태·로그·리소스 세 화면을 붙였다. 상태 카드를 누르면 그 서비스의 로그로, 로그 행의 추적 ID를 누르면 드로어에 구간 목록이 뜨는 식으로 이어진다",
          "트레이스 전용 화면은 만들지 않았다. 추적이 이벤트 파이프라인 한정이라 전용 화면은 과하다고 봤다",
          "배포도 나중에 직접 만든 배포 파이프라인으로 옮겼다. 처음엔 수동으로 파일을 복사해 올렸는데, 자산 위치와 컨테이너 이름을 플랫폼 규칙에 맞추면서 관측 스택도 다른 서비스와 같은 방식으로 배포된다",
        ],
      },
      {
        heading: "아직 비어 있는 곳",
        items: [
          "알림이 없다. 지금은 사람이 화면을 봐야 아는 상태라서, 개발 환경에서만도 디스크가 꽉 찬 것을 두 번 놓쳤다. 이벤트 파이프라인의 알림 채널에 붙이는 것이 다음 순서다",
          "헬스 엔드포인트가 없는 서비스가 아직 남아 상태 보드에 중단으로 떠 있다. 레포마다 스무 줄이면 되는 일인데 붙이는 순서에서 밀렸다",
          "지표 롤업과 로그 풀텍스트 인덱스는 범위에서 뺐다. 검색은 14일치 풀스캔을 허용하는 쪽으로 두고, 보존이 길어지면 다시 봐야 한다",
          "관리형 리소스 로그는 APIM만 가져온다. 나머지는 지표만 본다. 양 끝이 우리 코드인 구간은 간접 관측으로 충분하다고 판단했고, 필요해지면 그때 늘리기로 했다",
        ],
      },
    ],
  },
